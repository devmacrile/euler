...
def triangle_sums(nodes, row, index):
    if row == (len(nodes) - 1):
        return [nodes[row][index]]
    left_sums = [(nodes[row][index] + s) for s in triangle_sums(nodes, row + 1, index)]
    right_sums = [(nodes[row][index] + s) for s in triangle_sums(nodes, row + 1, index + 1)]
    return left_sums + right_sums

@clock
def main():
    nodes = load_triangle()
    return max(triangle_sums(nodes, 0, 0))
